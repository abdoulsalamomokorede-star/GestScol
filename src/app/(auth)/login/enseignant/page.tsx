  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile.role !== 'enseignant') {
        setError(t('toast.access_denied_role_teacher', "Accès refusé. Cet espace est réservé aux enseignants."))
        ...
        <div className="space-y-6 my-auto">
          ...
          <div className="space-y-4 text-start">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/35 text-primary dark:text-emerald-400 text-xs font-semibold border border-primary/20">
              ...
              <span>{t('login.teacher_space', "Espace Enseignant")}</span>
            </div>
            <p className="text-muted-foreground text-xs leading-relaxed hidden md:block text-start">
              {t('login.teacher_desc', "Saisissez vos notes en toute simplicité, gérez l'assiduité grâce à la feuille d'appel tactile rapide, et suivez vos classes.")}
            </p>
          </div>
          ...
          <Card className="w-full shadow-lg border-border/50 text-start">
            <CardHeader className="space-y-1 text-center bg-slate-50 dark:bg-slate-900/50 border-b border-border/50 dark:border-border/30 rounded-t-xl pb-4">
              <CardTitle className="text-xl font-bold text-primary dark:text-emerald-400">{t('login.title_teacher', "Espace Enseignant")}</CardTitle>
              <CardDescription>
                {t('login.desc_teacher', "Connectez-vous avec votre adresse e-mail professionnelle")}
              </CardDescription>
            </CardHeader>
            <form onSubmit={handleLogin}>
              <CardContent className="space-y-4 pt-6 text-start">
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    id="email" 
                    type="email" 
                    placeholder={t('login.placeholder_email_teacher', "ex: enseignant@ecole.com")} 
                    value={email}
              ...
              <CardFooter className="flex flex-col space-y-4">
                ...
                <div className="text-center text-xs text-muted-foreground pt-4 border-t border-border/40 w-full">
                  {t('login.not_teacher', "Vous n'êtes pas enseignant ?")}{" "}
